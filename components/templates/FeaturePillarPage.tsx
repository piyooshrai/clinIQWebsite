...
export interface FeaturePillarData {
  slug: string
  name: string
  publishDate?: string
...
// ── Content renderer ──────────────────────────────────────────────────────────
// Handles: - bullets, • bullets, numbered lists, bold headers, paragraphs.

function isBulletLine(l: string): boolean {
  return l.trimStart().startsWith('• ') || /^(\s*)-\s/.test(l)
}

function isNumberedLine(l: string): boolean {
...
function renderContent(content: string): ReactNode[] {
  ...
  return blocks.flatMap((block, idx) => {
    ...
    const otherLines = lines.filter((l) => !isBulletLine(l) && !isNumberedLine(l))

    // Bullet list block (with optional intro)
    if (bulletLines.length > 0 && numberedLines.length === 0) {
      const intro = otherLines.join(' ').trim()
      return [(
    ...
    // Numbered list block (with optional intro)
    if (numberedLines.length > 0 && bulletLines.length === 0) {
      const intro = otherLines.join(' ').trim()
      return [(
        <div key={idx} className={css.contentBlock}>
          {intro && <p className={css.contentP}>{renderInline(intro)}</p>}
          <ol className={css.contentUL}>
    ...
    if (trimmed.startsWith('**Related:')) {
      return [<p key={idx} className={css.relatedRef}>{renderInline(trimmed)}</p>]
    }

    return [<p key={idx} className={css.contentP}>{renderInline(trimmed)}</p>]
  })
}

// ── Component ─────────────────────────────────────────────────────────────────

export default function FeaturePillarPage({ data }: { data: FeaturePillarData }) {
  const faqSchema = {
